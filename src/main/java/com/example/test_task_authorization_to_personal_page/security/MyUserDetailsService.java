...
//    public UserDetails loadUserByUsername(String login) throws UsernameNotFoundException {
//        Person person = personRepository.findByLogin(login);
//        System.out.println(person.getLogin()+person.getRoles());
...
//        System.out.println( person.getRoles().stream().map(role -> new SimpleGrantedAuthority("ROLE_"+role))
//                .collect(Collectors.toList()));
//        return person.getRoles().stream().map(role -> new SimpleGrantedAuthority("ROLE_"+role))
//                .collect(Collectors.toList());
